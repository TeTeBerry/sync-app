...
import TicketTradeModal, { type TicketTradeMode } from "./TicketTradeModal";
...
const TicketMarketPanel: React.FC<TicketMarketPanelProps> = ({
  ...
  const filteredTickets = useMemo(() => {
    if (ticketFilter === `all`) return listings;
    return listings.filter((item) => item.type === ticketFilter);
  }, [listings, ticketFilter]);
      ...
      <div className={cn(`s-ticket-market`, className)}>
        <div className="s-aim-action-row">
          <Button
            ...
            onClick={() => setTicketModalMode(`sell`)}
          >
          ...
          <Button
            ...
            onClick={() => setTicketModalMode(`buy`)}
          >
