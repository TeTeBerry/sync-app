...
export function packageTierCtaLabel(tier: Pick<PackageTierDefinition, "id" | "name">): string {
...
export function resolvePackageTierStepCta(params: {
  ...
  if (activityPreKnown) {
    return resolvePackageActivityPayCta({ selectedTier, paidTierId: currentPaidTierId });
  }
  if (!selectedTier) {
