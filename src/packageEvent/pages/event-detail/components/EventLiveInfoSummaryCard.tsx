...
export function EventLiveInfoSummaryCard({
  ...
  const title = filterSubtitle?.trim()
    ? `现场综合快报 · ${filterSubtitle.trim()}`
    : '现场综合快报';

  return (
    <View className="s-live-info-card">
      <View className="s-live-info-card__head">
        <Text className="s-live-info-card__title">{title}</Text>
        <Text className="s-live-info-card__meta">基于 {certCount} 条认证数据</Text>
      </View>
      {safeRows.length === 0 ? (
        <Text className="s-live-info-feed__empty">当前筛选下暂无汇总数据</Text>
      ) : (
