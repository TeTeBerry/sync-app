...
export type ContactUnlockUpgradeCompareRowId = 'contactUnlock' | 'map';
...
function resolveFreeMonthlyContactLimit(freeMonthly?: FreeMonthlyQuota | null): number {
  return freeMonthly?.contactUnlock.limit ?? FREE_MONTHLY_CONTACT_UNLOCK_LIMIT;
}
...
function buildCurrentColumn(
  ...
  if (!paidTierId) {
    return {
      current: '',
      contactUnlock: formatMonthlyCount(resolveFreeMonthlyContactLimit(freeMonthly)),
      map: '限量',
    };
  }
