...
function inferZoneAndDayTags(text: string, tags: Set<string>): void {
  ...
  while (match) {
    addTag(tags, `${match[1]}号${match[2].toUpperCase()}区`);
    addTag(tags, `${match[2].toUpperCase()}区`);
...
function inferTicketResaleTags(text: string, tags: Set<string>): void {
  ...
  if (dotDate) addTag(tags, `#${dotDate[1]}.${dotDate[2]}`);

  const cnDate = text.match(/(\d{1,2})月(\d{1,2})日?/);
  if (cnDate) addTag(tags, `#${cnDate[1]}.${cnDate[2]}`);
}
...
export function inferIntentTagsFromText(...texts: Array<string | undefined>): string[] {
  ...
  for (const { pattern, tag } of GENDER_TAG_RULES) {
    if (pattern.test(haystack)) addTag(tags, tag);
  }
