...
function shouldSplitGenreList(value: string): boolean {
  ...
  const parts = withoutParens
    ...
    .filter(Boolean);
  return (
    parts.length > 1 && parts.every((part) => part.length <= MAX_GENRE_TOKEN_LENGTH)
  );
}
...
/** Sanitize catalog `styles` / `genres` arrays before lineup display. */
export function sanitizeCatalogGenreTokens(tokens: string[] | undefined): string[] {
