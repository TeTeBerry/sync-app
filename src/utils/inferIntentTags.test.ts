...
import { inferIntentTagsFromText } from './inferIntentTags';
...
describe('inferIntentTagsFromText', () => {
  ...
  it('infers ticket resale tags from折价出票 post', () => {
  ...
  it('merges multiple text inputs', () => {
    expect(inferIntentTagsFromText(undefined, '  ', '13A区', '有姐妹吗')).toEqual(
      expect.arrayContaining(['#女生', '#13号A区']),
    );
  });
});
