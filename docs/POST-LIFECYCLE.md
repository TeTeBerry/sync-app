...
2. 校验本活动是否有招募中帖（`userHasRecruitingBuddyPost`），无则引导先发帖
3. 打开 `TeamApplySheet`：展示 **与帖主帖最匹配** 的自己招募帖（`resolveUserBuddyPreviewForTargetPost`）
4. `POST /api/posts/:id/applications` — 可选 `message`
