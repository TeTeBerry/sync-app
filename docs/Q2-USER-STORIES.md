...
> **日韩攻略缺口（2026-06）**：境外场馆 **高德 `place/around` 不可用**（见 `travel-guide-amap-coverage.util.ts`）。现网 Hot Path 仅覆盖 Storm(4) / TML 泰国(1) / EDC 泰国(5)；**EDC Korea(8)** 等日韩场无兜底 POI → 生成接口 503。**上线前**用 Hot Path 精选数据收口，**不接** Google API（见 US-Q2-50 上线后）。
...
| ID | 标题 | 原因 |
|----|------|------|
| US-Q2-19 | 艺人 Tab → 活动串联 | ✅ Phase A/C 已交付；Phase B 待 US-Q2-33 |
...
| 序 | ID | 标题 | 体量 | 状态 | 说明 |
|----|-----|------|------|------|------|
| 1 | US-Q2-19 | 艺人 Tab → 活动串联 | S | ✅ | Phase A + 搜索/曲风/偏好排序 ✅；Phase B 待做 |
...
- 活动 Tab 地图视图上线
- 艺人 Tab 深度串联（US-Q2-19 Phase B/C-3）仍可按数据后置
- 详情内嵌购票按钮
...
- [ ] `TRAVEL_GUIDE_HOT_ACTIVITIES` 增加 **EDC Korea**：场馆坐标（seed 已有）· `readableAddress` · 枢纽路线（**仁川 ICN** · **首尔 GMP** · 可选金浦/弘大接驳文案）
- [ ] `interCityOrigins`：上海 / 北京 / 深圳 / 广州 → ICN 航班提示（规则文案，不调 direction API）
- [ ] `FALLBACK_BY_ACTIVITY` 增加 **8**：≥4 酒店 + ≥2 夜宵 POI（会场周边真实店名与估算坐标，附 `infoSource` 备注）
- [ ] `buildTravelGuideDocumentItems` / `buildTravelGuideEssentials`：**韩国**分支（护照、K-ETA、T-money、Kakao T / Uber、韩元现金等）
- [ ] `POST …/travel-guide/generate` legacyId=8 · 出发地「上海」→ **200** · 含 `accommodationSchemes` · `hotels` · `transportLines`
- [ ] LLM system prompt：境外场改为「候选来自地图检索或运营精选 POI」，**禁止**写「均来自高德」

**Phase B — 日韩 catalog 补齐（legacyId 3 · 6 · 11）**

- [ ] **S2O Korea (3)**：首尔乐园场馆 · 仁川/金浦枢纽 · 酒店/夜宵兜底 POI
- [ ] **WDJF Japan (6)**：海の森水上競技場 · 羽田 HND / 成田 NRT 枢纽 · 兜底 POI
- [ ] **Ultra Japan (11)**：台场场馆 · 羽田/成田枢纽 · 兜底 POI
- [ ] `buildTravelGuideDocumentItems` / `essentials`：**日本**分支（护照、Visit Japan Web、Suica/Pasmo、日元、Uber Japan 等）
- [ ] `TRAVEL_GUIDE_DEPARTURE_CITIES` 增加 `首尔` · `东京`（境外出发兜底，低优先级）

**Phase C — 体验与冒烟（D3 末 / D11 前）**

- [x] 未支持境外场：**隐藏**「生成出行攻略」或展示「该场攻略筹备中」（`findHotActivityProfile` / 兜底 POI 双检）；**禁止**裸 503
...
**砍 scope**：仅保 **Phase A（EDC Korea）** 仍可提审；Phase B 可延至 D4 buffer，但 catalog 日韩场不应再 503。
...
| **D2～D3** | 06-24～25 | W1 | **日韩攻略 Hot Path**（P0：先 EDC Korea · 再 S2O/WDJF/Ultra） | **Q2-49** |
...
**Q2-31** scene-run · **Q2-50** Google Places Provider · **Q2-43** 必看 set · **Q2-44** 申请评论 AI · **Q2-47** 官宣规律 · **Q2-19** 艺人串联（解阻后）· 行为看板 · **Q2-27** 偏好 A/B
