...
| 序 | ID | 标题 | 体量 | 目标日 | 状态 |
|----|-----|------|------|--------|------|
| 9 | US-Q2-27 | 用户偏好 Lite ✅ / Full 待做（`budgetLevel` · 列表开关） | M | D1 | 🚧 |
...
| 首页全站「回忆广场」/ 赛后图文 feed | ❌ 回忆仅绑单场活动详情折叠区（US-Q2-51） |
| 赛后回忆配图 / 九宫格晒图 | ❌ 首版纯文字；发帖链路现不支持公开图片 UGC |
...
**本周新收口（代码已合入 · 2026-06-23）**：**US-Q2-30** MVP ✅ · **US-Q2-49** Phase A+B+C ✅ · **US-Q2-19** Phase C ✅ · **US-Q2-33** 阵容点选 + 曲风导航 ✅ · **US-Q2-27** Lite ✅（Full 待做）· **US-Q2-17/18/23** 部分基线 🚧。
...
- [ ] 活动详情（阵容已官宣）有 **「投必看 Set」** 入口；进入投票页
- [ ] 阵容网格多选，**最多 3** 位；未满 3 可提交；已选 DJ 有选中态（复用 US-Q2-33 点选组件）
- [ ] 提交前须 **微信登录**；同一用户同一活动 **可改票 1 次/日**（防刷，可配置）
- [ ] 结果页：我的心愿（3 场）+ 全场 **TOP 10**（DJ 名 · 票数或占比 · 总投票人数）
- [ ] 主 CTA：**去招募墙找同行** → `event-detail?focusPosts=1`（默认 US-Q2-21 有种子的热门活动）
- [ ] 次 CTA：**发一条招募**（预填活动 · 可选带「必看 Set」标签到备注，非必填）
- [ ] 底栏：分享卡片 · 保存海报 · 再改心愿

**验收标准 — 分享与落地**

- [ ] `onShareAppMessage` 标题示例：`{活动名} · 我投了这 3 场 Set`（i18n 中英）
- [ ] 分享 path 携带 `share=1` · `activityLegacyId` · 分享者 pick ids（teaser）；**不携带**分享者 openid
- [ ] 好友点开：展示分享者 3 场心愿 + 全场 TOP3 teaser + **「我也来投」**
- [ ] 好友须 **自己完成投票并登录** 后，才可选写 `favorGenres`；点开未投完 **不算** 拿到其偏好
- [ ] 海报含活动名 · 个人 3 场 · TOP3 · 小程序码；底部合规小字（`platformDisclaimer` 摘要）

**验收标准 — 合规**

- [ ] 全场榜为聚合数据；个人页 / 分享卡 **不展示** 其他用户投票明细
- [ ] 无「匹配度」「缘分队友」「配对成功」「帮你找最佳搭子」表述
- [ ] `rg '匹配度|缘分队友|配对成功|最佳搭子' src/` 无新增命中
- [ ] UGC 相关字段（若备注预填）仍走联系方式拦截

**验收标准 — 工程**

- [ ] 后端：`POST /activities/:legacyId/set-votes`（提交/改票）· `GET .../set-votes/leaderboard`（聚合榜）
- [ ] 契约：`@sync/activity-contracts` 或新 `set-vote` 类型（`SetVotePick` · `SetVoteLeaderboardEntry`）
- [ ] 前端：投票页分包或阵容区 modal · `setVoteWechatShare.util.ts`（对齐 `personalityWechatShare.util.ts` 模式）
- [ ] 单测：改票限制 · 榜聚合 · 分享 path 编解码
- [ ] [RELEASE-SMOKE.md](./RELEASE-SMOKE.md) 增 1 条：官宣活动投票 → 分享 → 落地 → 招募墙
...
- [ ] 活动详情出征准备 / Festival Plan → **行程记账**（`my-itinerary`）入口可达
- [ ] 拍小票识别成功或手动保存金额后，弹出 **分摊 Sheet**：开关「参与分摊」· 人数 stepper
- [ ] 默认人数：攻略 `headcount`（从攻略页/详情跳转带入）> 记账页已设 N > **2**
- [ ] 确认后节点展示：`¥860 · 4 人 · 人均 ¥215`（或关闭分摊仅显示总额）
- [ ] 刷新/重进页面，分摊字段仍保留（走 `SaveTravelPlanPayload` 持久化）
- [ ] 顶栏 Stats：**预计花费** · **按 {N} 人估人均**（N = 记账页统一人数）

**验收标准 — 复制与合规**

- [ ] 「复制分摊摘要」：活动名 + 条目列表（标题 · 总额 · 人均）+ 「试算仅供参考，请线下自行结算」
- [ ] 记账页/Sheet 常驻：平台 **不经手资金、不担保** 结算
- [ ] `rg 'AA收款|代付|向队友收|组队账单|平台担保' src/` 无新增命中
- [ ] 小票图仍走云存储 + 内容安全；与现网 `recognize-receipt` 一致

**验收标准 — 工程**

- [ ] 扩展 `@sync/travel-plan-contracts`：`TravelPlanNodeRecord` 可选 `splitEnabled?` · `splitCount?`（`perPerson` 前端计算即可）
- [ ] 后端 `SaveTravelPlan` 读写新字段；旧节点无字段视为不分摊
- [ ] 单测：均分计算 · save/load 往返 · split 关闭时人均不展示
- [ ] [RELEASE-SMOKE.md](./RELEASE-SMOKE.md) §出征准备 增 1 条冒烟
...
- [ ] 主 CTA：**翻招募卡 / 去招募区找队**（`focusPosts` 或 US-Q2-29）
...
- [x] 测完 → US-Q2-27 写 `favorGenres` · US-Q2-17 次 CTA 预填招募帖 ✅ · 主 CTA 招募墙/翻卡 🔲
...
| **与招募帖分轨** | 新帖型 / tag（如 `memory` / `#回忆`），与 `#组队` 招募帖分列表展示；**不**混入 AI 找队默认检索 |
...
| **D2** | 06-24 | W1 | 人格测完 CTA · 分享卡片终检+埋点 · **Set 票选 MVP（启动）** · 结构化申请 Sheet · 发帖场景字段 | **Q2-17** 🚧 · **Q2-18** 🚧 · **Q2-53** 🔲 · **Q2-23** 🚧 · **Q2-24** |
...
3. **US-Q2-27** Full（或至少 Lite 已交付部分）  
4. **US-Q2-06** 口径终检 + 冒烟  
5. **合规 grep** + 提审材料  

**砍 scope 时的推迟顺序（从 D5 往前砍）**

D5 本地轴（37～40）→ D4 Scene（31～32）→ D3 增长（28～29）→ D2 体验（23～24）→ D2 人格裂变（17～18）→ **最后砍** Set 票选 TOP 榜（**Q2-53 仍保投票+分享+CTA**）→ **最后砍** Phase B 日韩场（Q2-49 仅保 EDC Korea Phase A）

### Sprint 6（上线后 1～2 周 · Scene + 转化 + 境外地图）

**Q2-31** scene-run · **Q2-50** Google Places Provider · **Q2-43** 必看 set · **Q2-44** 申请评论 AI · **Q2-47** 官宣规律 · **Q2-27** Full · 行为看板 · 偏好 A/B

### Sprint 7（增长 · 查节叙事 + 裂变）

**Q2-41** 节故事 MVP · **Q2-42** 哪场适合我 · **Q2-48** 生存指南 · **Q2-51** 赛后文字回忆（试点 1～2 场已结束热门节）· **Q2-39** Hub 跨场 API · 翻卡/发帖串联收口
