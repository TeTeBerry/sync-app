...
| US-Q2-17～18 · 28～30 · 29 | 人格串联 / AI 发帖 / 翻卡 / 攻略串联 | P2 增长，需上线后数据 |
...
**Story**  
作为找队用户，我希望系统用我在设置里保存的偏好（出发城市、曲风、预算），在 **AI 检索公开招募** 时对结果做优先排序；偏好可来自人格测试、出行攻略自动写入，也可在设置页修改。

**上线包拆分（W2 有余力）**

| 档位 | 范围 |
|------|------|
| **Lite（推荐上版）** | AI 找队结果 `insight_line` + 人格完成写 `favorGenres` |
...
- [ ] **热门全球电音节** 横滑区：`hot === true` 的 3～5 场，点击进入详情
...
**Story**  
作为已选活动的用户，我想在观演准备折叠区看到 **下一步建议**（如「去发招募」「生成攻略」），感到产品懂进度。

**验收标准**

- [ ] **上线包**：纯规则（读 Festival Plan checklist），无 LLM
- [ ] 文案示例：「还差：发一条公开招募」「你的招募有 N 条新公开回复」
- [ ] 点击跳转现有能力（发帖 Sheet / 攻略 / 详情招募区）
- [ ] （上线后）可接 Scene `prep_nudge` 增强

**技术提示**：`FestivalPlanSummaryBar` · `buildFestivalPlanChecklist` · `prep-guidance.util.ts`（后端参考）
...
3. **US-Q2-27 Lite** 洞察行（1d，可与 D9 并行）  
...
US-Q2-17 · 29 · 28 · 30 · 18 · 19；US-Q2-20 下线清理
