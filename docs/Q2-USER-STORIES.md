...
> **关联代码**：`domains/partner-feed/` · `post-search.service.ts` · `pages/events/` · `pages/index/` · `packageEvent/pages/event-detail/`  
> **无感 Agent**：[SCENE-AGENT.md](./SCENE-AGENT.md)（US-Q2-31～34）
...
现网已具备：`onShareAppMessage` / 朋友圈分享、分享 path（`share=1` · `primaryType` · `soulDjId`）、好友落地 teaser、海报分享、首页入口。**冷启动可先推卡片分享**；偏好写入与测完转化待 US-Q2-27 / 17 收口。详见 [PRODUCT.md §2.6](./PRODUCT.md#26-冷启动双轨)。
...
### 上线包 · 有余力再做（P1 · 差异化，不挡提审）

| 序 | ID | 标题 | 体量 | 目标周 | 状态 |
|----|-----|------|------|--------|------|
| 8 | US-Q2-27 | 用户偏好优先排序（洞察行 + 人格写偏好） | M | W2 | 🔲 |
| 9 | US-Q2-34 | 观演准备智能下一步（规则版 Prep Nudge） | XS | W2 | 🔲 |
| 10 | US-Q2-25 | 招募列表筛选（仅招募中 / 城市 chip） | S | W2 | 🔲 |
| 11 | US-Q2-26 | 招募进度帖主自维护（+1 / 改人数） | S | W2 | ✅ |
| 12 | US-Q2-35 | 出行攻略预算对比选档（生成后三卡 + 写偏好） | S | W2 | 🔲 |

> **资讯轴（US-Q2-36）** 与找队 P0 并列：对外一句含「电音节资讯」，提审前须完成活动 Tab 目录增强 + 详情资讯区（含 US-Q2-12 来源标注）。

### 明确推迟到上线后（Post-launch）

| ID | 标题 | 原因 |
|----|------|------|
| US-Q2-31 | Scene Agent `scene-run` API | 基建；上线包用现有 REST 映射 effect |
| US-Q2-32 · 33 | 动态 Chip · DJ 半屏卡片 | 依赖 Q2-31 或额外排期 |
| US-Q2-23 · 24 | 结构化申请 / 发帖场景字段 | 体量 M，不挡首版 |
| US-Q2-17～18 · 28～30 · 29 | 人格裂变转化 / 翻卡 / AI 发帖 / 攻略串联 | P2 增长；**18 分享基线现网已有**，17/27 为裂变闭环 |
...
### P1 — 上线后第 3～5 周（体验 + 无感 Agent 基建）

| 序 | ID | 标题 | 体量 | 状态 |
|----|-----|------|------|------|
| 6 | US-Q2-31 | Scene Agent 运行时（scene-run + effects） | L | 🔲 |
| 7 | US-Q2-23 | 申请加入结构化 Sheet | M | 🔲 |
| 8 | US-Q2-24 | 发帖场景字段（拼房/曲风/经验等） | M | 🔲 |
| 9 | US-Q2-32 | 招募墙动态筛选 Chip（Scene） | M | 🔲 |
| 10 | US-Q2-33 | 阵容 DJ 点选无感解读 | S | 🔲 |

### P2 — 有互动数据后（增长）

| ID | 标题 | 状态 | 前提 |
|----|------|------|------|
| US-Q2-17 | 人格测试 → 找队/发招募 CTA | 🔲 | 上线包有发招募/回复数据 |
| US-Q2-29 | 翻招募卡（人格测试串联） | 🔲 | US-Q2-17 · US-Q2-21 |
| US-Q2-28 | AI 辅助发招募（暗号 / 组队卡预览） | 🔲 | US-Q2-24 |
| US-Q2-30 | 攻略串联找队 / 发招募预填 | 🔲 | US-Q2-27 |
...
**Sprint 5（当前）**：**上线冲刺 ～2 周** — 种子帖 · 口径终检 · **全球节讯目录** · 冒烟提审 · 可选偏好洞察/Prep Nudge · 攻略预算对比选档（W2）。
...
| US-Q2-21 | 冷启动种子招募帖（运营） | **P0 必做** | 🔲 |
| US-Q2-23 | 申请加入结构化 Sheet | 上线后 | 🔲 |
| US-Q2-24 | 发帖场景字段扩展 | 上线后 | 🔲 |
| US-Q2-25 | 招募列表筛选/排序增强 | P1 可选 | 🔲 |
| US-Q2-26 | 招募进度帖主自维护 | P1 可选 | ✅ |
| US-Q2-27 | 用户偏好优先排序 | P1 可选 | 🔲 |
| US-Q2-28 | AI 辅助发招募（预览发布） | P2 | 🔲 |
| US-Q2-29 | 翻招募卡（人格串联） | P2 | 🔲 |
| US-Q2-30 | 攻略串联找队 / 发招募预填 | P2 | 🔲 |
| US-Q2-31 | Scene Agent 运行时 | 上线后 | 🔲 |
| US-Q2-32 | 招募墙动态筛选 Chip | 上线后 | 🔲 |
| US-Q2-33 | 阵容 DJ 无感解读 | 上线后 | 🔲 |
| US-Q2-34 | 观演准备 Prep Nudge | P1 可选 | 🔲 |
| US-Q2-35 | 出行攻略预算对比选档 | P1 可选 | 🔲 |
| US-Q2-36 | 全球电音节资讯目录 | **P0 必做** | 🔲 |
...
- 缺口：人格测试未同步 `favorGenres`；`budgetLevel` 未参与打分；前端未展示「已参考偏好」
- **冷启动**：裂变新用户测完写 `favorGenres` 后，首次 AI 找队即可感知偏好排序（与 US-Q2-18 / 17 串联）

**验收标准**

- [ ] 人格测试完成 → `mergeUserProfileHints` 写入 `favorGenres`（来自类型 `genreTags`，不覆盖用户手改）
- [ ] 出行攻略：**点选预算档后** `applyTravelGuideHints`（US-Q2-35）；发帖 `applyBuddyPostHints`（现网保持）
- [ ] `scoreBuddyPostPreferenceMatch` 增加 `budgetLevel` 弱信号（权重低于城市/曲风）
- [ ] AI 找队结果区展示一行：「已参考你的偏好：{城市} · {曲风} · {预算}」（无偏好则不展示）
- [ ] 文案为「找到 N 条合适的公开招募（已按偏好排序）」，**非**「匹配度」「配对成功」
- [ ] （可选 P1）招募列表开关：「按我的偏好优先展示」— **只排序不过滤** 帖子
- [ ] 设置页 `BuddyPreferencesSettings` 说明：偏好用于公开招募检索排序，可随时修改

**合规**

- [ ] 偏好仅影响**同一活动下公开帖**的展示顺序，不生成「适合你的队友」
- [ ] 用户可关闭「按偏好排序」（若做开关）
...
| **阵容深页** | `exclusive-itinerary` 分包，资讯区只提供入口 |
...
| **D6～D7** | 活动卡片：地区标签 + 阵容状态 badge（已官宣 / 待官宣） |
...
- [ ] 搜索框下 **地区 chip**：全部 / 国内 / 海外 / 港澳台（复用 `Activity.region`，逻辑可抽自 `EventsMapRegionTabs` / `useEventsActivityMap`）
- [ ] 可选 **时间 chip**（客户端）：即将开始 / 本月 / 热门（`hot`）
- [ ] **热门亚洲电音节** 横滑区：`hot === true` 的 3～5 场，点击进入详情
- [ ] 列表 / 日历 / 艺人视图文案与空状态符合「节讯目录」定位（无「问 AI」兜底）
- [ ] 搜索仍用 `filterActivitiesForEventsSearch`；haystack 扩展至 `alias`（有则）

**验收标准 — 活动卡片**

- [ ] 展示 **地区** 标签（国内 / 海外 / 港澳台）
- [ ] 有 `schedulePublished` 时展示 **阵容状态**（已官宣 / 待官宣）
- [ ] 主路径仍为 **点卡片进详情**（不在卡片上抢「组队」CTA）

**验收标准 — 活动详情资讯区**

- [ ] 导航下方、招募区 **上方** 增加资讯区（首屏可见，非折叠）
- [ ] 状态 pill：即将开始 / 进行中 / 已结束
- [ ] 摘要行：类型 · 地区 · 更新于（US-Q2-12）
- [ ] CTA **「查看阵容与时间表」** → `exclusive-itinerary`
- [ ] `schedulePublished === false` 时展示未官宣文案 + **订阅活动更新**（复用 US-Q1-06）
- [ ] 招募区（AI 找队 + 帖列表）仍在资讯区 **下方**，视觉层级不混

**合规**

- [ ] 无代购/订票/撮合表述；来源区含「仅供参考，以主办方为准」
- [ ] 不出现独立「资讯广场」或 UGC 新闻 feed

**技术提示**

- 前端：`pages/events/index.tsx` · `EventsSearchBar` 下 chip 行 · `EventCard` · `packageEvent/pages/event-detail/` 资讯区组件
- 后端：`activity.schema.ts`（`region` · `infoSource` · `infoUpdatedAt`）· 行程 `schedulePublished` 透出至详情 API
- 复用：`EventsMapRegionTabs` · `filterMappableActivitiesByRegion` · `exclusive-itinerary` · `subscribeToActivityUpdates`
...
1. 地区 chip + 列表 filter  
2. 详情资讯区 + US-Q2-12 来源  
3. 阵容入口 → `exclusive-itinerary`  

可砍：热门横滑、时间 chip、卡片 lineup badge（不挡提审底线）
...
**产品注记（2026-06）**：Checklist UI 固定顺序为 `攻略 → 行程 → 发招募`，但 **不代表推荐完成顺序**。专属时间表依赖官宣，组队可在官宣前进行；Prep Nudge（US-Q2-34）与人格偏好（US-Q2-27）按 [PRODUCT.md §2.5](./PRODUCT.md#25-festival-plan观演准备--降优先级) 分阶段推荐，不强制先完成行程再找队。
...
- [ ] `AiGuidePlanSheet` 不再强制选预算；文案说明「生成后可对比三档」
- [ ] 首次生成以 `standard` 为基线（或 `budgetTier: unset` 后端等价策略），**只调一次** `travel-guide/generate`
- [ ] 结果页展示 3 张预算对比卡：档名 + 每晚 hint（¥150–300 / ¥300–600 / ¥600+）+ 本次出行住宿 **估算区间**（可复用 `budgetTierHotelNightRanges` / `buildTravelGuideBudgetItems`）
- [ ] 用户点选某档后：该卡高亮；攻略 `budgetLabel` / 住宿段落与所选档一致；**就近/市中心** 子方案仍可见
- [ ] 点选后调用 `applyTravelGuideHints` → 用户 `budgetLevel`（`low \| medium \| high`）有变化才 PATCH
- [ ] 卡片与页脚含免责声明：「AI 估算仅供参考，请自行核实价格与预订」
- [ ] **不做**：三档各生成一份完整攻略；「平台推荐/最低价保证」；未点选时写入偏好

**合规**

- [ ] 无代购/订票/担保表述；区间文案为「约」「参考」
- [ ] 与 `AI_TRAVEL_GUIDE_DISCLAIMER` 一致
...
- [ ] 交互：卡背点击翻开 → 展示帖摘要（人数、标签、集合点、暗号、备注）
...
**Story**  
作为生成出行攻略的用户，我想用攻略里的出行条件 **预填 AI 找队搜索** 或 **预填发招募**，而不是平台黑盒「根据攻略匹配队友」。

**验收标准**

- [ ] 攻略详情页或生成成功页增加 CTA（二选一或并列）：
  - **「用这些条件找公开招募」** → 跳转活动详情，搜索框预填（出发地、人数、预算等，来自攻略表单槽位）
  - **「按出行计划发招募」** → 打开发帖 Sheet，`travelGuideFormToBuddyPrefill` 预填
- [ ] 预填内容为可编辑；用户确认后再搜/再发
- [ ] 预算偏好由 **US-Q2-35** 点选档位写入；城市等仍可在生成后 `applyTravelGuideHints`
- [ ] **不做**：攻略全文自动匹配队友、攻略页内嵌「匹配结果流」

**技术提示**：`travelGuideToBuddyPost.ts`（已有 util，接 UI）· `useEventDetailPostSearch` 支持初始 query · `ai-travel-guide/index.tsx`

**依赖**：US-Q2-35（预算偏好）· US-Q2-27（洞察/排序）· US-Q2-08 ✅
...
- [ ] 测完 → US-Q2-27 写 `favorGenres` · US-Q2-17 主 CTA 进招募墙
...
- [ ] 半屏卡片展示 `query_dj_info` 结果摘要
- [ ] 加载态「正在查艺人信息…」，非「AI 正在思考」
- [ ] 免责声明一行

**技术提示**：`query-dj-info.tool` · 活动详情阵容组件

**依赖**：US-Q2-31（或活动内单轮 REST 封装）

---

### US-Q2-34 · 观演准备 Prep Nudge P1 · XS · 🔲
...
- Checklist 展示顺序为 `攻略 → 行程 → 发招募`，但 nudge **按真实时间线分支**，不线性强制完成 itinerary 再找队
- 阵容 / timetable 未官宣时：**弱化或跳过**行程 nudge，**不阻塞**招募墙与 AI 找队
- 有人格偏好时，可提示「已可参考你的曲风偏好找公开招募」（联动 US-Q2-27 洞察行）

**验收标准**

- [ ] **上线包**：纯规则，无 LLM；输入含 Festival Plan checklist + 阵容是否官宣 + 是否有 `favorGenres` + 招募回复数
- [ ] 分阶段文案示例（命中最高优先级一条即可）：
  ...
  - 其余 → 按 checklist 下一未完成项（攻略优先于行程，**招募不强制排在行程之后**）
- [ ] 点击跳转现有能力（发帖 Sheet / 攻略 / 招募区 / `exclusive-itinerary` / 订阅）
...
| 天 | 周 | 事项 | ID / 文档 |
|----|-----|------|-----------|
| D1～D2 | W1 | 运营种子招募帖上线热门活动 | US-Q2-21 |
| D1～D2 | W1 | （可选）社群/KOL 推人格测试卡片分享（现网链路已有） | US-Q2-18 基线 |
| D1～D3 | W1 | 组队动态/通知口径终检 | US-Q2-06 |
| D3～D5 | W1 | `rg` 合规自检 · 类目材料 · 用户协议/免责声明 | US-Q2-01 |
| D5～D6 | W1 | **全球节讯目录**：活动 Tab 地区 chip + 热门横滑 | US-Q2-36 |
| D6～D7 | W1 | **全球节讯目录**：卡片 badge + 详情资讯区 + 来源标注 | US-Q2-36 · Q2-12 |
| D6～D7 | W1 | 活动详情主路径手工走通（找队→申请→发帖） | — |
| D8 | W1/W2 | 资讯轴联调 · 节讯冒烟 | US-Q2-36 |
| D8 | W2 | **攻略预算对比选档**：表单可选预算 + 三卡 UI | US-Q2-35 |
| D9 | W2 | **攻略预算对比选档**：点选写偏好 + 住宿区按档刷新 | US-Q2-35 |
| D8～D10 | W2 | 可选：偏好洞察行 + 人格写 `favorGenres` | US-Q2-27 |
| D8～D10 | W2 | 可选：Prep Nudge 规则版 | US-Q2-34 |
| D9～D10 | W2 | 可选：招募 chip 筛选 | US-Q2-25 |
| D11～D12 | W2 | `RELEASE-SMOKE.md` 全量冒烟 · 修 P0 bug | — |
| D13～D14 | W2 | 微信提审 · 预留驳回修改 buffer | — |

**人力极紧时（上线包只做 4 件）**

1. **US-Q2-21** 种子招募帖  
2. **US-Q2-36** 最小子集（地区 chip + 详情资讯区 + 来源）  
3. **US-Q2-06** 口径终检 + 冒烟  
4. **合规 grep** + 提审材料  

**W2 有余力时的推荐顺序（约 3～4 人日）**

1. **US-Q2-36** 完整版（热门横滑 + 卡片 badge + 时间 chip，～2d）  
2. **US-Q2-35** 攻略预算对比选档（2～2.5d）  
3. **US-Q2-27 Lite** 洞察行 + 人格写 `favorGenres`（1d，裂变偏好闭环，可与 D9 并行）  
4. **US-Q2-34** Prep Nudge（0.5d）  
5. **US-Q2-25** 招募 chip（1d，可砍）

### Sprint 6（上线后 2～4 周）

US-Q2-31 · 32 · 33 · 23 · 24；视数据启动 US-Q2-17 / 27 完整版 · **US-Q2-19** 艺人串联

### Sprint 7（增长 · 有互动数据后）

US-Q2-17 · 18（卡片视觉 + 裂变埋点）· 29 · 28 · 30 · 19；US-Q2-20 下线清理  
**注**：US-Q2-18 分享基线现网已有；Sprint 7 收口测完→招募墙转化与分享卡片终检。
