...
**Sprint 5（当前）**：**上线冲刺 ～2 周** — 种子帖 · 口径终检 · 冒烟提审 · 可选偏好洞察/Prep Nudge。
...
- [ ] 发帖时若填 `slotsTotal`，创建时显式写入 `slotsFilled = 1`（含帖主）
- [ ] 帖主在自己的招募卡上可操作：**「+1 人已确定」**、**「改人数」**（步进器）、保留现有 **「标为已满 / 重新招募」**
- [ ] 点「+1」前轻确认：「请确认已与对方沟通并确定同行；人数由你自行更新」
- [ ] `slotsFilled` 达 `slotsTotal` 时提示是否标为已满（可拒绝，后端 normalize 可作兜底）
- [ ] 进度条旁小字：「人数由帖主更新，仅供参考」
- [ ] **不做**：路人改进度、评论成功自动 +1、「接受申请」按钮

**合规**

- [ ] 无「XX 已加入你的队伍」「组队成功」文案
- [ ] 仍走 `PATCH /posts/:id/recruit`（仅帖主）

**技术提示**：`PostOwnerRecruitStatusButton` · `updatePostRecruit` · `buildRecruitFieldsFromBuddyForm`（默认 `slotsFilled: 1`）· `EventPostCard`
...
- [ ] 出行攻略 / 发帖继续 `applyTravelGuideHints` / `applyBuddyPostHints`（现网行为保持）
...
- [ ] 预填内容为可编辑；用户确认后再搜/再发
- [ ] 攻略生成继续 `applyTravelGuideHints`（城市、预算 → 用户偏好，US-Q2-27）
...
| D8～D10 | W2 | 可选：招募 chip 筛选 / 帖主 +1 进度 | US-Q2-25 · 26 |
...
US-Q2-31 · 32 · 33 · 23 · 24 · 12；视数据启动 US-Q2-17 / 27 完整版
