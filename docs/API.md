...
H5 devServer 将 `/api` 代理到 `http://localhost:3000`。  
...
**小程序生产路径**：`apiClient` 使用 `wx.cloud.callContainer`（见 [`utils/cloudRunTransport.ts`](../src/utils/cloudRunTransport.ts)）。**无需**在公众平台配置 `sh.run.tcloudbase.com` 为 request 合法域名。

本地联调 Nest：开发者工具勾选 **「不校验合法域名」**，可将 `TARO_APP_API_BASE_URL` 指向局域网 HTTP，并**清空** `TARO_APP_CLOUD_RUN_SERVICE`（或删除 `TARO_APP_CLOUDBASE_ENV_ID`）以回退 `Taro.request`。
...
X-Activity-Id: 4          # 可选，活动 legacyId（REST）
```
...
## Scene Agent（`POST /api/ai/scene-run`）

单轮 AI 任务入口；详见 [SCENE-AGENT.md](./SCENE-AGENT.md)。

| scene | 用途 |
|-------|------|
| `recruit_search` | 活动详情 AI 找队 |
| `recruit_compose` | 发帖 Sheet AI 帮写 |
| `events_knowledge_search` | 活动 Tab AI 搜节 |
...
| POST | `/api/ai/scene-run` | Scene Agent（`recruit_search` · `recruit_compose` · `events_knowledge_search`） |
...
| Scene AI | `src/domains/scene-agent/`、`src/api/sync/sceneRun.ts` |
