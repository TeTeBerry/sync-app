...
| POST | `/api/posts/:id/applications` | 申请加入。Body 可选：`message`；无招募帖时 `lightApply: { departureCity, tripDays?, genderPref? }`（`genderPref`: `女生优先` \| `男生优先` \| `不限`）。首条私信为合成正文；响应含 `teamChat` 供一键开聊 |
| POST | `/api/posts/:id/applications/:applicantUserId/accept` | 帖主接受组队 |
| GET | `/api/team-chats` | 当前用户相关的临时组队会话列表（申请人申请后或帖主发起沟通后出现；不含已过期） |
| POST | `/api/team-chats/:postId/:applicantUserId/open` | **帖主**从个人页发起沟通（写入 `ownerOpenedChatAt`；若尚无消息则写入申请附言） |
| GET | `/api/team-chats/:postId/:applicantUserId/messages` | 会话消息（申请人申请后或帖主发起后可读；过期返回 400） |
| POST | `/api/team-chats/:postId/:applicantUserId/messages` | 发送消息 `{ "body": "..." }`（同上） |
